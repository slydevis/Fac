 ...
 * renvoie les tokens sous forme d'entier. Le code de chaque unit̩ est d̩fini 
 * dans symboles.h sinon (mot cl̩, idententifiant, etc.). Pour les tokens de 
...
int yylex(void)
{
  ...
  /* Si j'ai trouv̩ la fin du fichier en lisant des espaces ... */
  if( !inTag && mangeEspaces() == -1 ) {
    return FIN; /* Renvoie le marqueur de fin d'entr̩e */
  }  
