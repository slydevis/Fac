...
int Node::maxHeight(Node *p) {
  if (!p) return 0;
  int leftHeight = maxHeight(p->m_leftNode);
  int rightHeight = maxHeight(p->m_rightNode);
  return (leftHeight > rightHeight) ? leftHeight + 1: rightHeight + 1;
}

void printBranches(int branchLen, int nodeSpaceLen, int startLen, int nodesInThisLevel, const deque<Node*>& nodesQueue, std::ostream& out) {
...
std::string Node::toString() const
{
    ...
    os      << "\n" << "Node : " << "{ m_key = "
            << m_key << ", m_h = " << m_h /* << ", leftNode = "
            << m_leftNode->m_key << " rightNode = "*/
            /*<< m_rightNode->toString() << "\n"*/ << "}";
